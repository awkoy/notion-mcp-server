...
- **`@hono/node-server` `1.19.14 → 2.0.12`.** It is a real runtime dependency in HTTP transport mode — the SDK's `StreamableHTTPServerTransport`, which `src/server/http.ts` imports, pulls `getRequestListener` from it. This clears the last open advisory (`GHSA-frvp-7c67-39w9`, path traversal in `serve-static` on Windows); `npm audit --omit=dev` now reports zero. Neither of v2's breaking changes touches us: the Node floor moved to `>=20`, which `engines` already declared, and the removed `@hono/node-server/vercel` adapter is not imported anywhere. The public API is unchanged. Lockfile-only — `@hono/node-server` is transitive, and #58 widened the SDK's range to `^1.19.9 || ^2.0.5` without moving the resolved version.
